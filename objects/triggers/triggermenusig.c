...
void
on_trg_create_activate(
    GtkMenuItem     *menuitem,
    gpointer         user_data)
{
    gtk_widget_show(create_trigger_window1());
}


void
on_trg_delete_activate(
    GtkMenuItem     *menuitem,
    gpointer         user_data)
{
    DirNode *dir = (DirNode *)user_data;
    ...
    if(confirm("You are about to drop this trigger.  This is a destructive operation "
               "that cannot be undone.  Do you know what you're doing?")){
	       
	drop_object(dbid, owner, object, "TRIGGER",1);
